...
def main():
    ...
    if fs is not None and audio_data is not None :
        print ('-=' *30)
        print ('\n Processamento do áudio pode continuar aqui...')
        ...
        # Calculo da dft :
        print (f'\n Iniciando o processamento DFT para os primeiros {N} samples do áudio...')
        X_complexo = transformada_Fourier(signal_block)
        print (f'\n Resultado da DFT (valores complexos) para os primeiros {N} samples :')
        print (X_complexo)
        # Cálculo da magnitude
        print ('\n Magnitude dos valroes da DFT :')
        result_magnitude = magnitude(X_complexo)
        print ('\n Resultados da magnitude :')
        ...
        print (f'\n A soma total do erro absoluto entre os dois métodos é : {np.sum(erro_absoluto)}')
        if erro_absoluto.item() < 1e-5: 
            print ("\n Validação bem sucedida! Os resultados estão de acordo com numpy.fft.")
        else :
        ...
        print ("\nErro na leitura do arquivo de áudio. Verifique o caminho e o formato do arquivo.")
